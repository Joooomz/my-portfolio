    ...
    <>
      <style>{`
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .exp-label-line { display: inline-block; width: 24px; height: 1px; background: #6effc0; flex-shrink: 0; }
        .exp-title {
        ...
        .exp-timeline { position: relative; z-index: 1; display: flex; flex-direction: column; margin-bottom: 4rem; }
        .exp-item { display: grid; grid-template-columns: 210px 1px 1fr; gap: 0 2.5rem; }
        .exp-period-col { display: flex; flex-direction: column; align-items: flex-end; padding-top: 0.15rem; gap: 0.5rem; }
        .exp-period { font-family: 'IBM Plex Mono', monospace; font-size: 0.68rem; letter-spacing: 0.1em; color: #6effc0; text-align: right; white-space: nowrap; }
        .exp-current-badge {
        ...
        .exp-line-col { display: flex; flex-direction: column; align-items: center; }
        .exp-node {
          width: 10px; height: 10px; border-radius: 50%;
          border: 1px solid #6effc0; background: rgba(110,255,192,0.15);
          box-shadow: 0 0 8px rgba(110,255,192,0.25);
          flex-shrink: 0; margin-top: 0.15rem; position: relative; z-index: 1;
        }
        .exp-node-dim { border-color: #2c2c44; background: rgba(44,44,68,0.4); box-shadow: none; }
        .exp-line { flex: 1; width: 1px; margin-top: 4px; }
        .exp-line-active { background: linear-gradient(to bottom, rgba(110,255,192,0.35), rgba(110,255,192,0.08)); }
        .exp-line-dim { background: linear-gradient(to bottom, #2c2c44, transparent); }
        .exp-content { padding-bottom: 3rem; }
        .exp-role {
          font-family: 'Bebas Neue', sans-serif;
          font-size: clamp(1.5rem, 2.5vw, 2.2rem);
          color: #dddde8; line-height: 1; margin-bottom: 0.2rem; letter-spacing: 0.02em;
        }
        .exp-company { font-family: 'IBM Plex Mono', monospace; font-size: 0.72rem; letter-spacing: 0.12em; color: #6effc0; text-transform: uppercase; margin-bottom: 0.15rem; }
        .exp-division { font-family: 'IBM Plex Mono', monospace; font-size: 0.62rem; letter-spacing: 0.08em; color: #88889a; margin-bottom: 1.1rem; }
        .exp-tags { display: flex; flex-wrap: wrap; gap: 0.4rem; margin-bottom: 1.25rem; }
        .exp-tag {
          font-family: 'IBM Plex Mono', monospace;
          font-size: 0.58rem; letter-spacing: 0.1em;
          text-transform: uppercase; color: #6effc0;
          background: rgba(110,255,192,0.06);
          border: 1px solid rgba(110,255,192,0.18); padding: 0.18rem 0.6rem;
        }
        .exp-highlights { display: flex; flex-direction: column; gap: 0.55rem; list-style: none; margin: 0; padding: 0; }
        .exp-highlight {
          display: flex; align-items: flex-start; gap: 0.5rem;
          font-size: 0.92rem; color: #88889a; line-height: 1.65;
        }
        .exp-highlight-icon { color: #6effc0; flex-shrink: 0; margin-top: 0.25rem; }
        .exp-certs-title {
          font-family: 'IBM Plex Mono', monospace;
        ...
        .exp-cert-year { font-family: 'IBM Plex Mono', monospace; font-size: 0.6rem; letter-spacing: 0.15em; color: #6effc0; text-transform: uppercase; }
        .exp-cert-name { font-family: 'Bebas Neue', sans-serif; font-size: 1.25rem; color: #dddde8; letter-spacing: 0.03em; line-height: 1.1; }
        .exp-cert-issuer { font-family: 'IBM Plex Mono', monospace; font-size: 0.58rem; color: #55556a; letter-spacing: 0.08em; }
        ...
        @media (min-width: 821px) { .exp-period-inline { display: none; } }
      ...
      <section ref={sectionRef} className="exp-section" id="experience">
        ...
        <p className="exp-label" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(16px)', transition: 'opacity 0.6s ease, transform 0.6s ease' }}>
          <span className="exp-label-line" />Career
        </p>

        <h2 className="exp-title" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(20px)', transition: 'opacity 0.7s ease 0.1s, transform 0.7s ease 0.1s' }}>
            ...
            <div key={i} className="exp-item" style={{
              ...
              <div className="exp-content">
                ...
                <div className="exp-tags">
                  {job.tags.map(tag => <span key={tag} className="exp-tag">{tag}</span>)}
                </div>
                <ul className="exp-highlights">
                  {job.highlights.map((h, j) => (
                    <li key={j} className="exp-highlight">
                      <ChevronRight size={13} strokeWidth={2} className="exp-highlight-icon" />
                      {h}
                    </li>
                  ))}
                </ul>
        ...
        <p className="exp-certs-title" style={{ opacity: inView ? 1 : 0, transition: 'opacity 0.7s ease 0.55s' }}>
          Certifications
        </p>
        <div className="exp-certs-grid" style={{ opacity: inView ? 1 : 0, transform: inView ? 'translateY(0)' : 'translateY(16px)', transition: 'opacity 0.7s ease 0.6s, transform 0.7s ease 0.6s' }}>
